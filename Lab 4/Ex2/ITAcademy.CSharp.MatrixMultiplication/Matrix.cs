            ...
            for (int row = 0; row < m1rows; row++)
            {
                for (int column = 0; column < m2columns; column++)
                {
                    ...
                    for (int cell = 0; cell < m1columns_m2rows; cell++)
                    {
                        ...
                        if (matrix1[cell, row] < 0) throw new ArgumentException($"Matrix1 contains an invalid entry in cell[{cell}, {row}].");
                        if (matrix2[column, cell] < 0) throw new ArgumentException($"Matrix2 contains an invalid entry in cell[{column}, {cell}].");
